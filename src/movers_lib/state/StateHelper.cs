...
public static class StateHelper
{
    ...
    {
        GlobalState.AddState(state);
        return FlagSuccess;
    }

    public static State GlobalStateRemove(State state)
    {
        GlobalState = (State)FormManager.GlobalState.Where(x => !state.ContainsKey(x.Key)).ToDictionary(x => x.Key, x => x.Value);
        return FlagFailure;
    }
    public static State GlobalStateRemoveColorScheme()
    {
        GlobalState = (State)GlobalState.Where(x => !x.Key.Any(y => y == COLOURSCHEME_CURRENT)).ToDictionary(x => x.Key, x => x.Value);
        return FlagFailure;
    }
    ...
    public static T Unwrap<T>(this State state)
    {
        ...
        if (val is null)
        {
            throw new Exception($"Could not unwrap null value");
        }
    ...
    public static IEnumerable<object> UnwrapAll(this State state)
    {
        ...
        if (val is null || val.Any(x => x is null))
        {
            throw new Exception($"Could not unwrap on a null value");
        }
